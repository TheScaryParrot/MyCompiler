...
Ab hunderttausend Kopien der Funktion kompiliert der THScompiler länger, als die beiden anderen Compiler.
...
Traditionelle Compiler haben die Möglichkeit Optimierungen auf Zwischencode auszuführen. Für meinem alternativen Compiler ist dies nicht möglich, da gar keine syntaktische Analyse durchgeführt und nie ein Zwischencode generiert wird.
Auch steht die Grundidee meines alternativen Aufbaus der Optimierung stark im Weg.
Wie zu Beginn von Abschnitt \ref{cha:4-QHS_Compiler} beschrieben, basiert mein Ansatz darauf, dass die von der \textit{Macro Expansion} verwendeten Macros erst während der Kompilierung definiert werden.
...
Traditionelle Compiler erscheinen dadurch manchmal etwas pingelig, sind dafür sehr genau und hilfreich beim Finden und Melden von Fehlern.
...
Mit dem QHScompiler lassen sich Libraries ebenfalls verwenden. Templates sollten theoretisch auch möglich sein, jedoch habe ich dies nicht getestet.
...
Daher werden auch für kurzen Code sehr viele \textit{Identifier} ausgeführt. Generell liesse sich die Implementation der \textit{Identifier} sicherlich stark verbessern und der gesamte QHScompiler optimieren.
