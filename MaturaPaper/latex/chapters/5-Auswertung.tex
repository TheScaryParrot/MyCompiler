...
Dazu gehören Variablen, Funktionen und \textit{Expressions} sowie Verzweigungen und Schleifen. Die Funktion wurde in die jeweiligen Sprachen übersetzt und mehrmals in das Programm eingefügt.
...
Wie zu Beginn von Abschnitt \ref{cha:4-QHS_Compiler} beschrieben, basiert mein Ansatz darauf, dass die von der \textit{Macro Expansion} verwendeten Macros während der Kompilierung erst definiert werden.
...
Leider ist die Definition der benötigten \textit{Identifier} für eine Programmiersprache nicht besonders intuitiv und benötigt Methoden, wie die Abschnitt \ref{sec:qhs-funcs} erklärten \textit{DelayedExecute} und \textit{TempAssign}.
