...
Dazu gehören Variablen, Funktionen und Expressions sowie If-Statements und Loops. Die Funktion wurde in die jeweiligen Sprachen übersetzt und mehrmals in das Programm eingefügt. Anschliessend wurde jedes Programm zehnmal kompiliert.
...
%Es ist möglich eigene \textit{Identifier} zu definieren. 
...
Leider ist die Definition der benötigten \textit{Identifier} für eine Programmiersprache nicht besonders intuitiv und benötigt Methoden, wie die Abschnitt \ref{sec:qhs-funcs} erklärten DelayedExecute und TempAssign.
...
Jeder \textit{Identifier} benötigt zuerst eine Abfrage bei den \textit{Environments}. Diese Abfrage ist an sich keine aufwendige Sache, jedoch sind \textit{Identifier} häufig sehr ineinander verschachtelt.
Daher werden auch für kurzer Code sehr viele \textit{Identifier} ausgeführt. Generell liesse sich die Implementation der \textit{Identifier} sicherlich stark verbessern und der gesamte QHScompiler optimieren.
...
Zusammengefasst führen während der Kompilerung definierte \textit{Identifier} zu hohen Kompierungsdauern, ungenauem Umgang mit Fehlern und mangelhafter Optimierung der Ausgabedatei.
